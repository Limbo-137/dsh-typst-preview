...
function highlightOptionsOf(config, tinymistPath) {
	return {
		tinymistPath,
		extraArgs: config?.extraArgs ?? DEFAULT_OPTIONS.extraArgs,
...
*/
function webSocketUrl(req, path) {
	const host = req.headers.host;
	if (host === void 0 || host === "") return path;
	...
	return `${proto === "https" || proto === "wss" || encrypted ? "wss" : "ws"}://${host}${path}`;
}
...
function apply(ctx, config) {
	...
	const highlighter = highlightEnabled ? new TypstHighlighter(highlightOptionsOf(config, previews.executable)) : void 0;
	...
	const pageRoute = {
		...
		handler: (req, res) => {
			...
			instance.lastUsed = Date.now();
			const wsUrl = webSocketUrl(req, `${WS_PREFIX}${instance.token}`);
