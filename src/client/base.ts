...
/** The transport global the shell publishes, as `@deepseek-ai/dsh-client-connection` declares it. */
interface TransportGlobal {
  readonly __DSH_TRANSPORT__?: { readonly streamBaseUrl?: string }
}
...
export function appUrl(path: string): string {
  const base = (globalThis as TransportGlobal).__DSH_TRANSPORT__?.streamBaseUrl
  if (typeof base !== 'string' || base === '') return path
  try {
    return new URL(path, base).href
  } catch {
    // A malformed base must not cost the user a preview: the host's own
    // same-origin fence still applies to whatever we send.
    return path
  }
}
