...
- **The desktop app only lets its own document carry a loopback socket.** Its window is served
  from `dsh-app://app/`, and the app's session cancels every `ws://127.0.0.1/*` handshake whose
  `Origin` is not `dsh-app://app` — rewriting the ones that are onto the host's real origin, with the
  host cookie and a same-origin fetch marker (`onBeforeSendHeaders` in the desktop main bundle). So
  the preview document stays on the app's own origin, and the socket it dials is written
  **absolutely** by the host (`ws://<host>/api/typst-preview/ws/<token>`): a relative socket URL under
  `dsh-app:` resolves to a scheme no WebSocket can be made from. In a browser both are ordinary
  same-origin URLs, and nothing is rewritten.
